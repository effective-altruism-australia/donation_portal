...
const charities = [
  ...
  // TODO: more. List of charities should be an AJAX JSON API call.
];


// TODO: one charity in the current set needs donors to go direct. Use the existing donation_portal approach (JSON) for this.
const charityOverrides = {
  "Against Malaria Foundation": {
    interject: <p className="donation-info">
...
// TODO: load the referral sources from AJAX JSON call
...
class CardPaymentDetails extends Component {
  ...
  componentDidMount() {
    // TODO: These reformat the numbers nicely as the user types BUT they don't account for
    // TODO: the cursor position. If a user types at the end of the input then it works well, but
    // TODO: if the user edits within the text, the cursor jumps to the end and it is not acceptable.
    //
    // TODO: The old stripe/jquery/payments code uses a function `saveVal()` that manipulated cursor
    // TODO: position intuitively. Look at "saveval()":
    // TODO: https://github.com/stripe/jquery.payment/blob/master/src/jquery.payment.coffee
    //
    // TODO: also handy: https://github.com/Gusto/react-masked-field/blob/master/src/SelectionUtils.js
...
class CardPaymentDetails extends Component {
  ...
  render() {
    let further = null;
    if (this.props.payment.method==='bank-transfer') {
      further = (
          <div>Bank stuff</div>
      )
    } else {
    ...
    return (
            ...
            <RadioGroup name="method" selectedValue={this.props.payment.method} onChange={this.props.handleInputChange}>
              <Radio value="credit-card"/>Credit Card
...
class DonationSubmit extends Component {
  render() { return (
    <div className="form-actions">
      <button type="submit" className="btn btn-success btn-lg" onClick={this.onSubmit}>Donate
...
class PaymentWizard extends Component {
  ...
  // setFrequency={(value, other) => console.log("set freq %s (%s)", value, other)
  render() {
...
class PledgeApp extends Component {
  constructor(props) {
    super(props);
  ...
  getInitialState() {
    let defaultCharity = null;

    // Uncomment this if you want a default charity during development
    //defaultCharity = charities[0];

    return {
      chosenCharity: defaultCharity,
