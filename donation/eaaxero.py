...
def xero_report_to_iterator(xero_report):
    ...
    headers = [cell["Value"] for cell in xero_report[0]["Rows"][0]["Cells"]]
    for row in xero_report[0]["Rows"][1]["Rows"]:
        yield dict(zip(headers, [cell["Value"] for cell in row["Cells"]]))
...
def import_bank_transactions(manual=False, tenant="eaa"):
    ...
    import_bank_transactions_from_account(
        settings.XERO_INCOMING_ACCOUNT_ID_DICT.get(tenant),
        from_date,
        to_date,
        manual,
        tenant,
...
def import_bank_transactions_from_account(
    bank_account_id, from_date, to_date, manual, tenant="eaa"
):
    passed_params = {
    ...
    try:
        bank_transactions = xero(tenant).reports.get(
            "BankStatement", params=passed_params
        )
    except XeroNotFound:
        if manual:
    ...
    for data in xero_report_to_iterator(bank_transactions):
        ...
        if data["Description"] in ["Opening Balance", "Closing Balance"]:
            continue

        bank_statement_date = data["Date"]
        bank_statement_text = data["Reference"]
        amount = data["Amount"]
        ...
        unique_id = hashlib.md5(
            "x".join([bank_statement_text, str(bank_statement_date), amount]).encode(
                "utf-8"
            )
        ).hexdigest()
        ...
        if rows_seen[unique_id] > 1:
            unique_id = hashlib.md5(
                (unique_id + str(rows_seen[unique_id])).encode("utf-8")
            ).hexdigest()
...
def import_trial_balance(org):
    ...
    while balance_date < date.today():
        ...
        trial_balance = xero(org).reports.get(
            "TrialBalance", params={"Date": balance_date}
        )

        for row in xero_report_to_iterator(trial_balance):
            try:
                account = Account.objects.get(date=balance_date, name=row["Account"])
            except Account.DoesNotExist:
                account = Account(date=balance_date, name=row["Account"])
            account.amount = to_decimal(row["Credit"]) - to_decimal(row["Debit"])
            account.ytd_amount = to_decimal(row["YTD Credit"]) - to_decimal(
                row["YTD Debit"]
            )
            account.save()
