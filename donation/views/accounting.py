...
# Notes: The total_donations_for_partner's aggregate operation was taking too long and preventing all other Django requests from being processed.
# The code below can be easily refactored to use async operations to prevent this from happening however they require newer versions of Django and Python.
# Once we upgrade, we'll be able to uncomment the total_donations_for_partner function and the bodies of the donation_counter and _accounting_reconciliation functions.
# For now, we can temporarily comment them back in when we need to run the accounting reconciliation (or if we need the donation counter agian).


def total_donations_for_partner(
    start_date, end_date, partner, payment_method=None, after_fees=False, card_type=None
):
    ...
    return (
        Donation.objects.filter(**filters)
        .annotate(
            amount_maybe_less_fees=F("components__amount")
            - (F("components__fees") if after_fees else 0)
        )
        .aggregate(Sum("amount_maybe_less_fees"))["amount_maybe_less_fees__sum"]
        or 0
    )
...
def donation_counter(request):
    ...
    xero_reconciled_date = XeroReconciledDate.objects.all().aggregate(Max("date"))[
        "date__max"
    ]
    ...
    error_message = ""
    if (
        xero_start_date <= xero_reconciled_date and xero_start_date.day != 1
    ) or arrow.get(xero_end_date).shift(days=1).date().day != 1:
        error_message = (
            "You must specify complete months for this period because of the way we do the accounting"
    ...
    totals = {
        partner.name: float(
            Account.objects.filter(
                date__gte=xero_start_date,
                date__lte=xero_end_date,
    ...
    unknown_total = BankTransaction.objects.filter(
        date__gte=django_start_date,
        date__lte=django_end_date,
    ...
    for account_description, account_name in [
        ...
        account_total = (
            Account.objects.filter(
                date__gte=xero_start_date, date__lte=xero_end_date, name=account_name
            ).aggregate(Sum("amount"))["amount__sum"]
            or 0
        )
        if account_total:
...
def _accounting_reconciliation(request, is_eaae):
    ...
    totals = {
        partner.name: {
            "bank": total_donations_for_partner(
                start, end, partner, payment_method="Bank transfer"
            ),
            "pin_payments": total_donations_for_partner(
                start, end, partner, payment_method="Pin"
            ),
            "pin_payments_after_fees": total_donations_for_partner(
                start, end, partner, payment_method="Pin", after_fees=True
            ),
            "stripe": total_donations_for_partner(
                start, end, partner, payment_method="Stripe"
            ),
            "stripe_after_fees": total_donations_for_partner(
                start, end, partner, payment_method="Stripe", after_fees=True
            ),
    ...
    grand_total = {
        kind: sum(total[kind] for total in totals.values())
        for kind in (
            "bank",
            "pin_payments",
    ...
    qs = BankTransaction.objects.filter(
        pledge__isnull=False, do_not_reconcile=True, is_eaae=is_eaae
    )
    if qs.exists():
        message = (
    ...
    exceptions = (
        BankTransaction.objects.filter(date__gte=start, date__lte=end, is_eaae=is_eaae)
        .exclude(pledge__isnull=False)
        .order_by("date")
    )
