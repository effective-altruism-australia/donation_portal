...
class PartnerCharityView(View):
    def get(self, request):
        charity_names = (
            PartnerCharity.objects.filter(active=True)
            .order_by("ordering")
            .values("slug_id", "name", "thumbnail", "category", "is_eaae")
        )
...
class ReferralSourceView(View):
    def get(self, request):
        referral_sources = (
            ReferralSource.objects.filter(enabled=True)
            .order_by("order")
            .annotate(value=F("slug_id"), label=F("reason"))
            .values("value", "label")
        )
