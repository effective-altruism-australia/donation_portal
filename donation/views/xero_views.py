...
def start_xero_auth_view(request):
    ...
    credentials = OAuth2Credentials(
        ...
        callback_uri="https://donations.effectivealtruism.org.au/process_callback",
        scope=[
...
def process_callback_view(request):
    context = {"success": False, "error_message": None, "tenant_info": None}

    try:
        cred_state = caches["default"].get("xero_creds")
        if not cred_state:
            context["error_message"] = (
                "No authentication state found. Please restart the login process."
            )
