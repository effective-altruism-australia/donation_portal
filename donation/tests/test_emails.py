...
from donation.emails import (
    email_receipt,
    send_bank_transfer_instructions,
    send_eofy_receipts,
    send_gift_notification,
    send_partner_charity_reports,
)
...
class EmailTestCase(TestCase):
    def setUp(self):
        ...
        self.bank_transaction = mommy.make(
            BankTransaction, amount=self.pledge.amount, pledge=self.pledge
        )
    ...
    def test_receipt(self):
        email_receipt.delay = MagicMock(
            side_effect=email_receipt
        )  # Don't send task to celery
        receipt = mommy.make(
            Receipt, bank_transaction=self.bank_transaction, pledge=self.pledge
        )
    ...
    def test_partner_charity_reports(self):
        ...
        self.assertIsNotNone(
            PartnerCharityReport.objects.get(
                partner=self.pledge.components.first().partner_charity
            ).time_sent
        )
