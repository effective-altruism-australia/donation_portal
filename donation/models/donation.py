...
class Donation(models.Model):
    ...
    bank_transaction = models.OneToOneField(
        BankTransaction, blank=True, null=True, on_delete=models.DO_NOTHING
    )
    pin_transaction = models.OneToOneField(
        PinTransaction, blank=True, null=True, on_delete=models.DO_NOTHING
    )
    stripe_transaction = models.OneToOneField(
        StripeTransaction, blank=True, null=True, on_delete=models.DO_NOTHING
    )
    ...
    def component_summary_str(self):
        return ", ".join(
            "%s ($%s)"
            % (
                component.pledge_component.partner_charity.name,
...
class DonationComponent(models.Model):
    ...
    pledge_component = models.ForeignKey(
        PledgeComponent, related_name="donation_component", on_delete=models.DO_NOTHING
    )
    donation = models.ForeignKey(
        Donation, related_name="components", on_delete=models.DO_NOTHING
    )
