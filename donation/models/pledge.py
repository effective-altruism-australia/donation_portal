...
class ReferralSource(models.Model):
    ...
    reason = models.CharField(
        max_length=256,
        help_text="Instead of editing this text, you probably want to "
...
class Pledge(models.Model):
    ...
    subscribe_to_updates = models.BooleanField(
        default=False, verbose_name="Send me news and updates"
    )
    ...
    how_did_you_hear_about_us_db = models.ForeignKey(
        ReferralSource,
        blank=True,
        null=True,
    ...
    subscribe_to_newsletter = models.BooleanField(
        default=False,
        verbose_name="Subscribe me to the global Effective Altruism newsletter",
    )
    connect_to_community = models.BooleanField(
        default=False,
        verbose_name="Connect me with my local Effective Altruism community",
    )
    ...
    def partner_charity_str(self):
        ...
        if num_components == 1:
            ...
            partner_names = [
                component.partner_charity.name for component in self.components.all()
            ]
    ...
    def generate_reference(self):
        ...
        # TODO
        self.reference = "".join(
            random.choice("ABCDEF" + string.digits) for _ in range(12)
        )
        self.save()
        return self.reference
    ...
    def get_is_eaae(self):
        ...
        assert (
            len(s) == 1
        ), "Pledge had partner charities from both EAA and EAAE.  This shouldnt happen"
        return s.pop()
    ...
    def org_name(self):
        return (
            "Effective Altruism Australia Environment Limited"
            if self.is_eaae
            else "Effective Altruism Australia Limited"
        )
    ...
    def donor_portal(self):
        return "https://donations.effectivealtruism.org.au" + reverse(
            "donor-portal-eaae" if self.is_eaae else "donor-portal-eaa",
            kwargs={"customer_id": self.stripe_customer_id},
        )

    def __str__(self):
        components = ", ".join([c.__str__() for c in self.components.all()])
        return (
            "Pledge of {1} by {0.first_name} {0.last_name}, "
            "made on {2}. Reference {0.reference}".format(
                self, components, self.completed_time.date()
            )
        )
...
class PledgeComponent(models.Model):
    ...
    pledge = models.ForeignKey(
        Pledge, related_name="components", on_delete=models.CASCADE
    )
    partner_charity = models.ForeignKey(
        PartnerCharity, related_name="pledge_components", on_delete=models.CASCADE
    )
