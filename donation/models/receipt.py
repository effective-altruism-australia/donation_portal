...
class Receipt(models.Model):
    ...
    bank_transaction = models.ForeignKey(
        BankTransaction, blank=True, null=True, on_delete=models.SET_NULL
    )
    pin_transaction = models.ForeignKey(
        PinTransaction, blank=True, null=True, on_delete=models.SET_NULL
    )
    ...
    )  # So the donor can download the receipt from our website.
    ...
    def pdf_receipt_location(self):
        return os.path.join(
            settings.MEDIA_ROOT, "receipts", "EAA_Receipt_{0}.pdf".format(self.pk)
        )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pin_transaction and not self.secret:
            self.secret = "".join(
                random.choice(string.letters + string.digits) for _ in range(16)
            )
    ...
    def status(self):
        if self.sent:
            return "Receipt to {0.email} sent at {1}".format(
                self, arrow.get(self.time_sent).format("YYYY-MM-DD HH:mm:ss")
            )
        elif self.failed:
    ...
    def __str__(self):
        if self.bank_transaction:
            transaction_part = (
                "Receipt for bank donation of ${0.bank_transaction.amount}"
                + " on {0.bank_transaction.date}"
            ).format(self)
        elif self.pin_transaction:
            transaction_part = (
                "Receipt for pin donation of ${0.pin_transaction.amount}"
                + " at {0.pin_transaction.date}"
            ).format(self)
        elif self.stripe_transaction:
            transaction_part = (
                "Receipt for stripe donation of ${0.stripe_transaction.amount}"
                + " at {0.stripe_transaction.date}"
            ).format(self)
        else:
        ...
        failed_part = (
            " - Sending failed: {0.failed_message}".format(self) if self.failed else ""
        )
