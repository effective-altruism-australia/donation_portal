...
class RecurringFrequencyField(EnumChoiceField):
    def clean(self, value):
        ...
        mapping_dict = {"one-time": None, "monthly": RecurringFrequency.MONTHLY}
        return mapping_dict[value]
...
class PledgeForm(forms.ModelForm):
    ...
    how_did_you_hear_about_us_db = forms.ModelChoiceField(
        queryset=ReferralSource.objects.all(), to_field_name="slug_id", required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.referral_sources = ReferralSource.objects.filter(enabled=True).order_by(
            "order"
        )
...
class PledgeComponentForm(forms.ModelForm):
    ...
    partner_charity = forms.ModelChoiceField(
        queryset=PartnerCharity.objects.all(), to_field_name="slug_id"
    )
    pledge = forms.ModelChoiceField(queryset=Pledge.objects.all(), required=False)


PledgeComponentFormSet = forms.modelformset_factory(
    PledgeComponent,
    form=PledgeComponentForm,
    fields=("pledge", "partner_charity", "amount"),
)
...
class PinTransactionForm(forms.ModelForm):
    ...
    date = forms.DateTimeField(
        required=False
    )  # Allow the pin save method to complete this field for us
