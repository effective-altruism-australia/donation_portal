...
async def test_tipping_submit_basic_donation_with_no_tip():
    """
    Ensure that the form sends the right data when a basic donation with no tip is submitted.
    """
    ...
    async with async_playwright() as p:
        ...
        await page.get_by_text("Skip", exact=True).click()
        ...
        assert request_data["form-TOTAL_FORMS"] == 1
        ...
        assert "form-1-id" not in request_data
        assert "form-1-partner_charity" not in request_data
        assert "form-1-amount" not in request_data
