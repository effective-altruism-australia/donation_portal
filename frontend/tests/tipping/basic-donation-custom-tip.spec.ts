...
/*
Ensure that the form sends the right data when a basic donation with a custom tip is submitted.
*/

test("Tipping: submit with basic donation and custom tip", async ({ page }) => {
  ...
  const testFinished = new Promise<void>((resolve) => {
    page.on("request", async (request) => {
      if (request.url().includes("pledge_new")) {
        ...
        expect(data["form-1-amount"]).toBe("20.00");
